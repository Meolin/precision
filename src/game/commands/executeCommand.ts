...
export function applyCannonCommand(
  ...
  if (
    'entityIds' in command ? !command.entityIds.includes(cannon.id) : command.cannonId !== cannon.id
  )
    return;
...
export function executeCommand(state: GameState, config: GameConfig, command: GameCommand): void {
  ...
  for (const id of ids) {
    ...
    if (!installation || !isInstallation(installation) || !isControllable(installation, playerId))
      continue;
    ...
    if (command.type === 'fire') {
      ...
      installation.fireControl.lastFailure = null;
      installation.fireControl.status = fireInstallation(state, config, installation)
        ? 'fired'
        : 'idle';
      continue;
    }
    ...
    if (command.type === 'attackGround') {
      ...
      if (
        ...
        x < 0 ||
        y < 0 ||
        x >= config.world.widthMeters ||
        y >= config.world.heightMeters
      )
        continue;
