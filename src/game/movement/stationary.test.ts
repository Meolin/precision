...
describe('stationary installations and support', () => {
  it('spawns four installations per owner without terrain or entity overlap', () => {
    ...
    for (const unit of state.units) {
      ...
      expect(circleTouchesTerrain(state.terrain, hitboxCenter(unit.position, unit.hitbox), unit.hitbox.radiusMeters)).toBe(false);
      for (const other of state.units) if (other.id !== unit.id)
        expect(Math.hypot(unit.position.x - other.position.x, unit.position.y - other.position.y))
  ...
  it('settles vertically into a crater and reports missing support without moving X', () => {
    ...
    expect(circleTouchesTerrain(state.terrain, unit.position, unit.hitbox.radiusMeters)).toBe(false);
  ...
  it('accounts for offset circles and updates all installations after terrain changes', () => {
    ...
    expect(circleTouchesTerrain(state.terrain, hitboxCenter(result.position, state.cannon.hitbox), state.cannon.hitbox.radiusMeters)).toBe(false);
    state.terrain.removeCircle({ x: 70, y: 45 }, 4);
    stepSimulation(state, config, 1 / 60);
    for (const unit of state.units.filter(isInstallation)) expect(unit.grounding.terrainVersion).toBe(state.terrain.version);
  });
});
