...
describe('bounded solver uses real projectile physics', () => {
  it.each([
    ...
    'reaches a sampled trajectory point with $windAcceleration wind and $airDrag drag without mutating state',
    (physics) => {
      ...
      for (let tick = 0; tick < 90; tick++)
        advanceProjectile(probe, state.terrain, config, 1 / 60, undefined, [state.cannon]);
      const target = { ...probe.position };
      const before = createGameSnapshot(state);
      const solution = solveBallisticAim(state.cannon, target, state.terrain, config, [
        state.cannon,
      ]);
      expect(solution).not.toBeNull();
      expect(solution!.missDistanceMeters).toBeLessThanOrEqual(config.rts.solverToleranceMeters);
      expect(
        solveBallisticAim(state.cannon, target, state.terrain, config, [state.cannon]),
      ).toEqual(solution);
      expect(createGameSnapshot(state)).toEqual(before);
      const live = createProjectile(
        { ...state.cannon, angleRad: solution!.angleRad },
        config,
        999,
        0,
      );
      ...
      for (let tick = 0; tick < 1200 && live.alive; tick++) {
        const hit = advanceProjectile(live, state.terrain, config, 1 / 60, undefined, [
          state.cannon,
        ]);
        nearest = Math.min(
          nearest,
          Math.hypot(live.position.x - target.x, live.position.y - target.y),
        );
        if (hit) break;
      }
