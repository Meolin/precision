...
export function filterControllableIds(
  ids: readonly EntityId[],
  units: readonly UnitState[],
  playerId: PlayerId,
): EntityId[] {
  return [...new Set(ids)]
    .filter((id) => units.some((unit) => unit.id === id && isControllable(unit, playerId)))
    .sort((a, b) => a - b);
}
...
export function clickSelection(
  ...
  if (!unit || !isControllable(unit, playerId))
    return additive ? [...selection.selectedEntityIds] : [];
